...
{
  "session_id": "...",
  "comment": "这手挺冲的，我得防一下了。",
  "grounding": { "last_user_move_facts": {...}, "last_ai_move_facts": {...} }
}
```
...
| 情形 | 行为 |
|---|---|
| 关键时刻：任一方 `created_chain>=3` / `blocked_*_chain>=3` / `has_four` / `has_open_three` / `winner` 非空 | 必评，跳过概率与冷却 |
...
**生成方式**：复用 `_build_messages`，把末段的 `用户说：…` 换成系统指令（不判断胜负、不超过 40 字、依据 `<game_facts>`）。**只写** `assistant_chat`（含 `proactive` / `at_move`），不写 `user_chat`。经过与 `/chat` 相同的输出清洗（Brief 43 §A）与 holdback 过滤；observe capture 的 `kind="comment"`（Brief 43 §B）。
...
| `admin/routers/gomoku.py` | HTTP API 路由（含 /ai_move、/comment） |
