...
| `/memory-events/search`、`/memory-events/{event_id}`、`/memory-events/{event_id}/window`、`/memory-events/{event_id}/related`、`/memory-events/query-trace`、`/memory-events/lineage/*` | Memory Event 证据账本的 scoped 只读检索、确定性关联边、派生记忆血缘和脱敏查询审计 | `admin-only`；`memory.read`，请求必须显式 `uid + char_id + realm=reality`；lineage 仅消费落盘的事件 ID，旧数据/已删除事件为 `legacy_unknown`，dry-run 不写回；管理面消费，桌面/手机不消费，不进入 prompt 或 tool loop |
