...
def _relation_prompt(char_a: str, char_b: str, excerpt: str, old: dict) -> str:
    ...
    return (
        ...
        # 追到这里发现摘要本身没有语气约束）——这段摘要会被原样当作"既有印象"注入
        # 每一轮群聊 prompt（core/stage/context.py::render_presence），角色会当真，
        # 所以措辞必须克制中性，不能替互动本身没有的暧昧色彩打底。
        "措辞保持中性、克制的第三人称观察口吻（例如「觉得对方靠谱」「合作时很默契」）；"
        "除非互动内容本身明确含有暧昧/亲密表达，否则不要主动往暧昧、亲密方向措辞，"
        "更不要臆测互动之外的感情。\n"
